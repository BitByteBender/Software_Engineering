...
void storeRows(uint16_t arr[3][3], uint16_t newArr[3], uint16_t rows)
{
  uint16_t i = 0;
  
  for (i = 0; i < rows; i++) {
    newArr[i] = sumRows(arr, i, rows);
  }
}

void printRows(uint16_t arr[3][3], uint16_t newArr[3], uint16_t rows)
{
  uint16_t i = 0;
  
  cout<<"The following are the sum of each row in the matrix:\n";

  storeRows(arr, newArr, 3);
